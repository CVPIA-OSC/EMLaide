...
* A file that contains the data (preferably csv if tabular data)
...
personnel_table <- metadata$personnel
...
# parent_element <- list(parent_element, personnel)
# 
# parent_element2 <- add_pub_date(parent_element = list(), date = NULL)
# apply_personnel = function(x){
#   parent_element2 = add_personnel(parent_element = parent_element2,
...
# }
# 
# element_list = apply(X = personnel_table, MARGIN = 1, FUN = apply_personnel)
# 
# 
# purrr::map_df(personnel_table, add_personnel(parent_element = parent_element, 
#                                   first_name = personnel_table$first_name, 
#                                   last_name = personnel_table$last_name, 
#                                   email = personnel_table$email,
#                                   role = personnel_table$role, 
#                                   organization = personnel_table$organization,
#                                   orcid = personnel_table$orcid))
# args <- list(personnel_table$first_name, personnel_table$last_name, personnel_table$email,
#              personnel_table$role, personnel_table$organization, personnel_table$orcid)
# purrr::pmap(list(personnel_table$first_name, personnel_table$last_name, personnel_table$email,
#              personnel_table$role, personnel_table$organization, personnel_table$orcid), .f = add_personnel)
# ```
# ```{r}
# parent_element <- list()
# first_name <- personnel_table[['first_name']]
...
# orcid <- personnel_table[["orcid"]]
# 
# args <- list(first_name, last_name, email, role, organization, orcid)
# results <- purrr::pmap_int(args, add_personnel)
# 
# 
# results[1]
```
...
if (length(!is.na(metadata$keywordThesaurus)) == 0) {
...
## Attempted rewrite using map 
# CVPIA_common_species = current[['CVPIA_common_species']]
# kingdom_value = current[['kingdom_value']]
# phylum_value = current[['phylum_value']]
# class_value = current[['class_value']]
# order_value = current[['order_value']]
...
# 
# 
# new_taxon <- purrr::pmap(list(CVPIA_common_species, 
#                kingdom_value, 
#                phylum_value, 
#                class_value, 
#                order_value, 
...
#                species_value, 
#                common_name, 
#                taxon_id), add_taxonomic_coverage)
# 
# new_taxon
```
