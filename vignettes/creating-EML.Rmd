...
The entity description is a short description for the datatable that you uploaded. 
...
entity_description <- "Hannon Snorkel Survey"
...
#### Project title
```{r}
project_title <- metadata_sheets$project_title$project_title
```


#### Project personnel 
```{r}
project_personnel_table <- metadata_sheets$project_personnel
...
```{r}
funding <- metadata_sheets$funding
award_information <- add_funding(funder_name = funding$funder_name,
...
If more methods are needed, you can create separate word documents.
The methods can be applied here as well as in the data table element,
...
The taxonomic coverage information is added to the "template-metadata.xlsx" 
...
if (length(taxonomic_coverage) ==  0) {
  taxonomic_coverage <- list(taxonomicClassification = "No informaion on taxonomic coverage")
}
if (length(taxonomic_coverage) ==  1) {
  taxonomic_coverage <- taxonomic_coverage
}
if (length(taxonomic_coverage) == 2) {
  taxon_1 <- taxonomic_coverage[1]
  taxon_2 <- taxonomic_coverage[2]
  taxonomic_coverage <- list(taxon_1, taxon_2)
}
if (length(taxonomic_coverage) == 3) {
  taxon_1 <- taxonomic_coverage[1]
  taxon_2 <- taxonomic_coverage[2]
  taxon_3 <- taxonomic_coverage[3]
  taxonomic_coverage <- list(taxon_1, taxon_2, taxon_3)
}
if (length(taxonomic_coverage) == 4) {
  taxon_1 <- taxonomic_coverage[1]
  taxon_2 <- taxonomic_coverage[2]
  taxon_3 <- taxonomic_coverage[3]
  taxon_4 <- taxonomic_coverage[4]
  taxonomic_coverage <- list(taxon_1, taxon_2, taxon_3, taxon_4)
}
```
...
working EML file. If you wish to append multiple datatables to one dataset please 
...
be equal to one that you reserved on the EDI data portal. Input this at the top of the 
