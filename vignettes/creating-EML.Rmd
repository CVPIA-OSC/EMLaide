---
title: "Creating EML Using EMLaide"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Creating EML Using EMLaide}
...
To follow along with this example please download the [Hannon-Example zip folder](https://cvpia-data-stewardship.s3-us-west-1.amazonaws.com/hannon-example.zip). 
...
dataset_file_name <- "snorkel_index_example_data.csv"
```
...
It is also possible to use the EDIutils function `api_create_reservation()` to generate a EDI number using R. See the `api_create_reservation()` [documentation.](https://ediorg.github.io/EMLaide/reference/api_create_reservation.html)
```{r}
# edi_number <- api_create_reservation(scope = "edi", environment = "staging", user.id = "your user id", 
...
work up to appending the more complex sections like `dataTable`. Sections will correspond with different sheets of metadata inputs from the `metadata.xlsx` document. 
...
[initial metadata information](https://cvpia-osc.github.io/EMLaide//articles/Inital-Metadata-Information.html).
...
to your file. If you are not adding a CC0 or CCBY default license please see [initial metadata information](https://cvpia-osc.github.io/EMLaide//articles/Inital-Metadata-Information.html) This information can be added to the 
...
instructions on appending multiple datatables please see the [dataset specific instructions](https://cvpia-osc.github.io/EMLaide//articles/Dataset-Element.html).
...
```{r}
physical <- add_physical(file_path = dataset_file, data_url = "s3://cvpia-data-stewardship/Hannon-Example/hannon_example_physical_data.csv")
```
...
attribute_list <- purrr::map(attribute_list, na.omit)
```
If your dataset contains units that are not part of the EML schema you may add them as custom units. To do so please see the vignette on [custom units.](https://cvpia-osc.github.io/EMLaide//articles/custom-units.html) 
...
[detailed dataset instructions](https://cvpia-osc.github.io/EMLaide//articles/Dataset-Element.html). 
