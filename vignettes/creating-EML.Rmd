...
![Alt text](figures/EML_diagram (2).jpeg){width=100%}
...
To follow along with this example please download the [Hannon-Example]()
...
#### Project personnel 
The creator of the dataset is appended as the `project_personnel`. If a different person is associated with the project please add that information to the "project" tab in the excel file "example-metadata.xlsx". The project personnel also requires that a role be specified in the list. Here we add `role = "Project Contact"`.
```{r}
#TODO is it better to assign the role within the add_project? Or to fix add_personnel function and do it within?
project_personnel <- personnel$creator 
project_personnel$role <- "Project Contact"
```
...
```{r}
add_funder <- function(funder_name, funder_identifier, award_number, award_title, 
                         award_url, funding_description) {
  award_information <- add_funding(funder_name, funder_identifier, award_number, award_title,
                                award_url, funding_description)
}

award_information <- purrr::pmap(metadata$funding, add_funder) %>% flatten()
```
