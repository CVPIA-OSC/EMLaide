...
* **default\license** - Select the default license if applicable to your project. 
...
required and only need to be filled out if you do not selevt a **CVPIA\_common\_species**.
...
* **attribute\_label (optionally)** -  A descriptive label that can be used to display the name of an attribute. 
* **domain** -  Please select a domain from the dropdown. Domain options vary depending on measurment\_scale. See the measurement\_scale sections below to see which domains are appropriate for each measurement\_scale. 
...
* **minimum (conditionally required)** - Required for measurement\_scale: *Ratio*, *Interval*, *Date Time*. Appropriate minimum values vary depending on measurment\_scale. See the measurement\_scale sections below to see what values are appropriate for each measurement\_scale.
