...
metadata template directory. Within the top level of the directory you
...
CVPIA data managers at [ecain\@flowwest.com](mailto:ecain@flowwest.com),
[erodriguez\@flowwest.com](mailto:erodriguez@flowwest.com),
or [sgill\@flowwest.com](mailto:sgill@flowwest.com).
...
All other information will be inputted into metadata.xlsx. Each section
below corresponds to a sheet of the xlsx document. Columns that are gray
are optional but every other column must be filled out. Some columns
...
different metadata.xlsx for each data table. If methodology or abstract
differ between the data tables, you can add subsections with headers
...
* **geometry** - (**Required only if the dataset is type vector)** EML schema accepts eight geometry values: Point, LineString, LinearRing, Polygon, Multipoint, MultiLineString, MultiPolygon, MultiGeometry.
...
* **keyword** - The keyword
* **keywordThesaurus** - *(Required only when keyword is from a controlled vocabulary)* A string identifying the controlled vocabulary the keywords originate from, do not include if keywords are not from a controlled vocabulary
...
with the dataset you must provide information for the following columns:

* **license\_name** - The official name of the license that applies to the data and metadata
* **license\_url** - The persistent URL for the license
* **license\_identifier** - The official identifier for the license, which should be drawn from the SPDX license list, or a similar well-known license registry
* **intellectual\_rights\_description** - Description of rights under the license
...
fill out all sections of this sheet.
* **first\_name** - Person's given name
* **last\_name** - Person's given last name
* **email (optional)**- Person's email address
* **role** - Select the appropriate role from the dropdown, contact CVPIA data manager if you would like a role added to the template  options
* **organization** - The name of the organization that the person is employed by
* **orcid (optional)** - ORCID iD is a persistent digital identifier for researchers, register at  [http://orcid.org/](http://orcid.org/)
...
* **funder\_name** - organization or individual providing the funding
* **funder\_identifier** **(Optional)** - CVPIA specific finding identifiers are in development
* **award\_number (Optional)** - The identifier assigned by the> funding agency to identify this funding award.
...
* **update\_frequency -** if status is "ongoing" provide the frequency of which the project or dataset is updated. Available update frequencies: daily, weekly, monthly, annual
...
set of taxa to the CVPIA common taxa list.
...
Please fill in at least the three following columns for each attribute
of your dataset within the 'attribute' tab:

* **attriubute\_name** - Name of the attribute as it appears in the dataset
...
* **date\_time\_precision** - To what level time is being measured
