---
title: "Generating an EML document for multiple datasets"
...
```
## Overview 
This vignette walks you through our process of using EDIutils to create a working EML 
document to upload to the EDI data repository. If you wish to use this workflow 
to generate your own EML doc you must first format your metadata to be consistent
with our example "~/data-raw/Hannon-Example/example-metadata.xlsx". Your dataset files 
may then be added in place of our example files by adding your file 
paths into the first two sections of code. 
...
TODO: Redo with an example that actually has multiple datasets (this one just appends the same one twice)

## Required Files 
The next two code blocks define the metadata and datasets that will be used by the rest of the
document to generate a working EML document. 

Metadata must be prepared following the structure of the "~/data-raw/metadata_template". Each dataset must have a separate metadata excel sheet. However, one metadata excel sheet will be used to create the overall metadata and the other metadata sheets will only be used to fill in the information specific to the dataset. 

Three metadata files are loaded in below:
...
Now we must add in the actual datasets and the metadata referring to the attributes of each dataset. 
...
### Data Tables 
Next, we need to add each data table. The data table element includes a `physical` section, an `attribute_list`, 
...
The loops below create a `data_tables` list where we append each data table. When looping through the 
for loop we create the `attribute_list` using the `add_attribute` function and the `physical` 
...
data_tables <- list()
for (h in 1:length(dataset_files)) {
  dataset_file <- dataset_files[[h]]$datatable
  dataset_file_name <- dataset_files[[h]]$datatable_name
  attribute_table <- dataset_files[[h]]$attribute
  codes <- dataset_files[[h]]$code
  dataset_methods <- dataset_files[[h]]$dataset_methods
  additional_info <- dataset_files[[h]]$additional_info  
  attribute_list <- list()
  ...
  if (!is_null(dataset_methods)) {
    dataTable$methods <- add_method(dataset_methods)
  }
  
  if (!is_null(additional_info)) {
    dataTable$additional_info <- additional_info
  }
...
Now we must append the `data_tables` list of datatables to the `parent_element`.
```{r}
