...
`add_keywords`, and `add_license` elements. This document will not explain how to create a complete valid EML document to upload to the EDI repository. Please use this article just for sections of the EML document and refer to [the EML template helper](https://cvpia-osc.github.io/EMLaide//articles/creating-EML.html) to generate the entire document.
...
add_keywords = function(unique_thesaurus){
  keywords <- add_keyword_set(keywords, 
                              keyword_set = list(keyword = metadata$keyword_set$keyword[metadata$keyword_set$keywordThesaurus == unique_thesaurus],
