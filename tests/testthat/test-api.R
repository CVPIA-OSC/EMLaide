...
test_that("evaluate_edi_package() returns the anticipated response object", {
  expected_return <- structure(list(Status = c("valid", "valid", "valid", "valid", 
                                               ...
                                               "valid", "valid", "valid", "valid", "valid", "info", "info", 
                                               "valid", "valid", "warn", "valid", "valid", "valid", "valid", 
                                               "valid"), 
                                    `Element Checked` = c("packageId pattern matches \"scope.identifier.revision\"", 
                                                          ...
                                                          "Dataset title length is at 5 least words.", "Dataset abstract element is a minimum of 20 words", 
                                                          "There are no duplicate entity names", "Length of entityName is not excessive (less than 100 char)", 
                                    ...
                                    `Suggestion to fix/imporve` = c("", "", "", "", "", "", "", "", "If sampling EML is used within methods, does that obviate geographicCoverage? Or should those sites be repeated or referenced?", 
                                                                    "Could search title, abstract, keywords for any taxonomic name (huge). Could search keywordType=\"taxonomic\".", 
                                                                    "Most datasets have a temporal range.", "", "", "", "Declare a non-empty entityName and ensure that there are no duplicate entityName values in the document", 
                                                                    "", "", "", "Add 'numFooterLines' element if needed.", "", "", 
                                                                    "Add a numberOfRecords element and store the number of entity records", 
                                                                    "Include an entity size in bytes", "Add a physical/authentication element and store the entity checksum or hash value in it using a method such as MD5 or SHA-1.", 
