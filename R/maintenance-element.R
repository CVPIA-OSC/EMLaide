...
#' @param update_frequency Only needed if the status of the project or dataset is ongoing. If this is the case, please provide the frequency of which the project or dataset is updated.  
#' @return The dataset or project with maintenance information appended.
#' @examples 
#' add_maintenance(parent_element = list(), status = "complete")
#' 
...
add_maintenance <- function(parent_element, status, update_frequency = NULL) {
  ...
  if(status == "ongoing") {
    if (is.null(update_frequency)){stop('Please provide the frequency of when this project or dataset is updated.', call. = FALSE)}
