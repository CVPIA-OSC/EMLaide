...
reserve_edi_id <-
  function(user_id,
    ...
    base_url <-
      dplyr::case_when(
        environment == "staging" ~ "https://pasta-s.lternet.edu/package/reservations/eml/edi",
    ...
    if (response$status_code == "201") {
      ...
      stop(
        "RequestFailed",
        paste("Your request to reserve an EDI number failed,
...
evaluate_edi_package <-
  ...
  if (response$status_code == "202") {
    ...
    if (response$status_code == "200") {
      ...
      message("Please check for errors in the report dataframe")
      return(report_df)
      break
    } else {
      stop("RequestError", paste("Request failed, check that you inputs are all valid and try again. See more
           information on request status below:", response))
    }
      # if (max_iter > iter) {
        # stop("TimeOutError"," Request timed out, check that you inputs are all valid and try again")
      # }
    # }
  } else {
    stop("RequestError", paste("Your request to evaluate an EDI package failed,
...
upload_edi_package <-
  ...
  if (response$status_code == "202") {
    ...
    if (message == "Attempting to insert a data package that already exists in PASTA") {
      stop("PackageError", "Attempting to insert a data package that already exists in PASTA. Please reserve a different identifier or update the existing package using update_edi_package()")
    }
    # the EML is not valid - we must view errors in error report dataframe
      ...
      stop("EMLError", "EML not valid. Please fix errors in report dataframe or if report dataframe comes back empty please try to evaluate_edi_package().")
      # break
    } else {
      # iter <- 0
      # max_iter <- 5
      ...
      if (check_upload$status_code == "200") {
        message("Your data package posted to EDI. Please check EDI portal to confirm")
        break
      } else {
        stop(
          "RequestError",
          paste("Request failed, check that you inputs are all valid and try again. 
          See more information on request status below:", response)
        )
        }
      }
    ...
    stop(paste("Your request to upload an EDI package failed,
           please check that you entered a valid username, password, and XML document.
           That XML document must link to a csv accessible online.
           See more information on request status below:", response))
  }
...
update_edi_package <-
  ...
  if (response$status_code == "202") {
    ...
    if (message == paste0("Attempting to update a data package to revision ", "'", revision_number, "' ", "but an equal")) {
      stop("VersionError", "Attempting to insert a version that already exists in PASTA. Please reserve a different identifier or update to the next revision")
    }
    # the EML is not valid - we must view errors in error report dataframe
    else if (check_error$status_code == "200" & 
      ...
      if (check_upload$status_code == "200") {
        message(
          paste(
            "Your data package posted to EDI. Please check EDI",
            environment,
            "portal to confirm"
          )
        )
    ...
    stop(paste("Your request to update an EDI package failed,
           please check that you entered a valid username, password, and XML document.
           That XML document must link to a csv accessible online.
           See more information on request status below:", response))
  }
